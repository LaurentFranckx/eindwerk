...
"Lines of text (in millions)" <- c(sprintf("%1.1f", length(or_tables[["blogs"]])/10^6 ),sprintf("%1.1f", length(or_tables[["twitter"]])/10^6 ),sprintf("%1.1f", length(or_tables[["news"]])/10^6 ))

"Word count (in millions)" <- c(sprintf("%1.1f", TestFreqList[["blogs"]]$Count/10^6 ),sprintf("%1.1f", TestFreqList[["twitter"]]$Count/10^6 ),sprintf("%1.1f", TestFreqList[["news"]]$Count/10^6 ))

c(sprintf("%1.1f", nrow(TestFreqList[["blogs"]]$LowDF)/10^3 ),sprintf("%1.1f", nrow(TestFreqList[["twitter"]]$LowDF)/10^3 ),sprintf("%1.1f", nrow(TestFreqList[["news"]]$LowDF)/10^3 ))

knitr::kable(SumTable, caption = "Distribution of unique words counts")

#GlobFreqTable$logMinOccur <- log10(GlobFreqTable$MinOccur)

```

The "uncleaned" version of the "blogs"", "Twitter"" and "news"" table contain, resp `r sprintf("%1.1f", length(or_tables[["twitter"]])/10^6 ), ` millions lines of text. 

When we limit the table to alphanumeric characters, it contains a total of
`r sprintf("%1.0f", TestFreqList[[2]]/10^6) ` million occurences of words, with  `r sprintf("%1.0f", nrow(GlobFreqTable)/1000) ` thousand unique words.

Out of these unique words, `r sprintf("%0d", nrow(TestFreqList[[3]])) ` words (this is thus `r sprintf("%1.2f", nrow(TestFreqList[[3]])/nrow(GlobFreqTable) * 100) ` %) correspond to 90% of all word occurences.
