...
#most_freq_words <- sapply(corpusnames, function(x)   freq_tables[[x]][ freq_tables[[x]]$freq == max(freq_tables[[x]]$freq) , "word" ])
most_freq_2gr <- sapply(corpusnames, function(x)   freq_tables2Gr[[x]][ freq_tables2Gr[[x]]$freq %in% tail_table2gr[, paste(paste("# of counts in", x)) ] , "word" ])
...
Thus, for instance, `r sprintf("%1.0f",head_table2Gr[1, 1]/1000)` thousand individual 2-grams appear exactly `r head_table[1, 2]` time in the Twitter database, while `r head_table2Gr[nrow(head_table2Gr), 1]` individual 2-grams  appears exactly `r head_table2Gr[nrow(head_table2Gr), 2]` times in the Twitter database.

For the three databases the 6 most frequent 2-grams are:

```{r, echo=FALSE, cache=2}
knitr::kable(most_freq_2gr , caption= "Most frequent words in the three databases")
```
...
```{r, echo=FALSE, cache=2}
set.seed(123)
fewwords <- lapply(corpusnames, VeryInfWords, tables = freq_tables2Gr)
do.call("cbind",fewwords)
