...
The English database itself is composed of three files, each composed of data sets containing several hundreds of thousands of text lines from blogs, news sites and Twitter. 
...
We notice that while the Twitter dataset contains approximately three times the number of lines of text as the other datasets, it contains less words, which is of course due to the restrictions imposed on the length of individual tweets. On the other hand, the Twitter dataset contains more *unique* words, reflecting probably idiosyncratic expressions and abbreviations used by the "tweeters". 

We also observe that, in all three datasets, the vast majority of word occurences originates from a small (data set specific) dictionary. The general skewness of the frequency of words is confirmed when we look at the summary statistics of the number of counts:  
...
These statistics confirm that the vast majority of unique words appears just a few times in the total database, but that there are some words that occur with a very high frequency. This is also illustrated by the plot in appendix, which shows how the share of individual words in the set of unique words changes when we require that they appear at least, respectively, 1, 10, 20, 30, 40 or 50 times in the respective complete dataset. We see a sharp initial decrease. For instance, in all three datasets, the share of words occuring at least ten times is between (approxilately) 15 and 25% of all unique words. 



Still another way to look at the data is to calculate how many individual words appear in the database with a given count. For instance: how many *unique* words appear exactly 1, 2, 3 etc times? We represent here the first five and the last five lines of the table containing this information (henceforth the "frequency count table"). 
...
```{r, echo=FALSE, cache=2}
head_table <- cbind(head(CounttableTwit,3),head(Counttableblogs,3))
head_table <- cbind(head_table,head(Counttablenews,3))

tail_table <- cbind(tail(CounttableTwit,3),tail(Counttableblogs,3))
tail_table <- cbind(tail_table,tail(Counttablenews,3))
...
names(head_table) <- c("# of words in twitter", "# of words in blogs", "# of words in news", "# of counts")
knitr::kable(head_table, caption = "Low frequency words")

row.names(tail_table) <- NULL 
tail_table <- tail_table[ , c(1,2,4,6)]
names(tail_table) <- c("# of counts", "# of words in twitter", "# of words in blogs ", "# of words in news " )
knitr::kable(tail_table, caption = "High frequency words")

```

Thus, for instance, `r sprintf("%1.0f",head_table[1, 1]/1000)` thousand individual words appear exactly `r head_table[1, 4]` time in the Twitter database, while `r tail_table[nrow(tail_table), 1]` word  appears exactly `r tail_table[nrow(tail_table), 2]` times in the Twitter database.
...
None of these are surprising, though it should bo noted that the appearance of "you" and "i" in the Twitter top 6 and of "i" in the blogs top 6 are an indication of the more "personnal" contents of these datasets. 
...
VeryInfWords <- function( corpus, tables, lessthan = 5, samplesize = 5){
...
In the plot in the appendix, we have set out the logs of the variables in the "frequency count table". This confirms that, for words that appear very frequently, the observations deviate substantially from the predictions of a power law. 
...
We see thus that the vast majority of 2-gram occurences originates from a small set of *unique* 2-grams. However, as we should have expected, the share of these *unique* 2-grams in the total dataset is much larger (a factor twenty) than for individual words.   
...
```{r, echo=FALSE, cache=2}
head_table2Gr <- cbind(head(CounttableTwit2Gr,3),head(Counttableblogs2Gr,3))
head_table2Gr <- cbind(head_table2Gr,head(Counttablenews2Gr,3))


tail_table2gr <- cbind(tail(CounttableTwit2Gr,3),tail(Counttableblogs2Gr,3))
tail_table2gr <- cbind(tail_table2gr,tail(Counttablenews2Gr,3))
...
names(head_table2Gr) <- c("# of 2-grams in twitter", "# of 2-grams in blogs", "# of 2-grams in news", "# of counts")
knitr::kable(head_table2Gr, caption = "Low frequency 2-grams")

tail_table2gr <- tail_table2gr[  , c(1,2,4,6)]
row.names(tail_table2gr) <- NULL
names(tail_table2gr) <- c("# of counts", "# of 2-grams in twitter", "# of 2-grams in blogs", "# of 2-grams in news")
...
Thus, for instance, `r sprintf("%1.0f",head_table2Gr[1, 1]/1000)` thousand individual 2-grams appear exactly `r head_table2Gr[1, 4]` time in the Twitter database, while `r tail_table2gr[nrow(tail_table2gr), 1]` individual 2-grams  appears exactly `r tail_table2gr[nrow(tail_table2gr), 2]` times in the Twitter database.
...
```{r, echo=FALSE}
aa <- qplot(MinOccur, data= GlobFreqTable2, geom = "bar", fill = time, weight = Share, position = "dodge", binwidth = 5) + xlab("Minimal occurence of word in dataset") + ylab("Share in total number of unique words")

ab <- qplot(MinOccur, data= GlobFreqTable2Gr2, geom = "bar", fill = time, weight = Share, position = "dodge", binwidth = 5) + xlab("Minimal occurence of 2-gram in dataset") + ylab("Share in total number of unique 2-grams")
...
##The frequency count tables for the individual words and 2-grams


```{r, echo=FALSE, cache=2}
a <- qplot(log10(CounttableTwit[,2]),log10(CounttableTwit[,1])   , data= CounttableTwit) +xlab("log(Number of words)") + ylab("log(Counts in Twitter)") 
b <- qplot(log10(Counttableblogs[,2]),log10(Counttableblogs[,1])   , data= Counttableblogs) +xlab("log(Number of words)") + ylab("log(Counts in blogs)") 
c <- qplot(log10(Counttablenews[,2]),log10(Counttablenews[,1])   , data= Counttablenews) +xlab("log(Number of words)") + ylab("log(Counts in news)") 
library(grid)
grid.newpage()
...
```{r, echo=FALSE, cache=2}
d <- qplot(log10(CounttableTwit2Gr[,2]),log10(CounttableTwit2Gr[,1])   , data= CounttableTwit2Gr) +xlab("log(Number of 2-gr)") + ylab("log(Counts in Twitter)")   
e <- qplot(log10(Counttableblogs2Gr[,2]),log10(Counttableblogs2Gr[,1])   , data= Counttableblogs2Gr) +xlab("log(Number of 2-gr)") + ylab("log(Counts in blogs)") 
f <- qplot(log10(Counttablenews2Gr[,2]),log10(Counttablenews2Gr[,1])   , data= Counttablenews2Gr) +xlab("log(Number of 2-gr)") + ylab("log(Counts in news)") 
library(grid)
grid.newpage()
pushViewport(viewport(layout = grid.layout(2, 2)))
vplayout <- function(x, y)
