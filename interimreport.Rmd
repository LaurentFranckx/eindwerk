...
We also observe that, in all three datasets, the vast majority of word occurences originates from a small (data set specific) dictionary. The general skewness of the frequency of words is confirmed when we look at the summary statistics of the number of counts (see Appendix).  

These statistics confirm that the vast majority of unique words appears just a few times in the total database, but that there are some words that occur with a very high frequency. This is also illustrated by the plot in Appendix, which shows how the share of individual words in the set of *unique* words changes when we require that they appear at least, respectively, 1, 10, 20, 30, 40 or 50 times in the respective complete dataset. We see a sharp initial decrease in this share. For instance, in all three datasets, the share of words occuring at least ten times is between (approxilately) 15 and 25% of all unique words. 


Still another way to look at the data is to calculate how many individual words appear in the database with a given count. For instance: how many *unique* words appear exactly 1, 2, 3 etc times? In Appendix, we represent  the first five and the last five lines of the table containing this information (henceforth the "frequency count tables"). 
...
None of these are surprising, though it should be noted that the appearance of "you" and "i" in the Twitter top 6 and of "i" in the blogs top 6 are an indication of the more "personnal" contents of these datasets. 
...
Finally, we can have a look at a random sample of words that appear very infrequently in the database (here, less than 5 times) - see the table in Appendix. These words are generally meaningless in English, or are obvious spelling mistakes.  
...
However, a formal hypothesis has rejected this possibility for the databases we use here (details are available on request). In the plot in the Appendix, we have set out the logs of the variables in the "frequency count table". This confirms that, for unique words that appear very frequently, the observations deviate substantially from the predictions of a power law. 
...
The general skewness of the frequency of 2-grams is confirmed when we look at the summary statistics of the number of counts (see Appendix). 
...
A table in Appendix gives the most frequent 2-grams for each dataset.
...
Finally, we can have a look at a sample of 2-grams that appear very infrequently in each database (see Appendix). We see that there are still a lot of these 2-grams that make little sense in English, but compared to individual words, there are also some "low frequency" 2-grams that do make sense.
...
```{r, echo=FALSE, cache=2}
knitr::kable(most_freq_2gr , caption= "Most frequent 2-gram in the three datasets")
```
